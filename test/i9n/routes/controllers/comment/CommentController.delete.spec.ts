...
describe('Comment controller delete', () => {
  ...
  test('Delete comment successfully. As regular user', async () => {
  ...
  test('Delete comment successfully. As administrator after 1 hour', async () => {
    const knex = container.resolve<Knex>('DBConnection')
    const uid = uuidv4()
    await knex('comment').insert({
      uid,
  ...
  test('Delete comment failed. Non exist uid', async () => {
    const uid = uuidv4()
    ...
    expect(response.body.message).toBe('Not found [comment] entity for delete')
    expect(response.body.source).toBe('comment')
    expect(response.body.code).toBe(404002)
  })

  test('DELETE comment failed. Non uuid uid', async () => {
    const response = await request(TestContext.app)
      .delete(`${endpoint}/non-uuid`)
      .set('Cookie', [`token=${regularUserData.jwtToken}`])
  ...
  test('Delete comment failed. As regular user after one hour', async () => {
    const knex = container.resolve<Knex>('DBConnection')
    const uid = uuidv4()
    await knex('comment').insert({
      uid,
